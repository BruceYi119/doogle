...
public class AdminNoticeController {
	...
	public String notice(Model model, HttpServletRequest request, NoticeDTO dto, HttpSession session) {
		...
		if (dto.getTitle() == null || dto.getTitle() == "") {
			...
			paging.setPaging(page, noticeMapper.getTotal("where title = #{title}", dto.getTitle()),
					"/admin/notice?title=" + dto.getTitle());
			model.addAttribute("list", noticeMapper.getAll(paging.getStartRow(), paging.getViewCnt(),
					"where title = #{title}", dto.getTitle()));
			System.out.println(dto.getTitle());
		}
	...
	public String notice_update(Model model, int no, HttpSession session) {
		if (!member.isAdminLogin(session))
			return "redirect:/login";
	...
	public String notice_view(Model model, int no, HttpSession session) {
		if (!member.isAdminLogin(session))
			return "redirect:/login";
