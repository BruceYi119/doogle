...
public interface ProductMapper {
	...
	@Select({
			"select pno, price, discount, earn, ctno, ctno1, ctno2, quantity, name, brand, subject, sel_unit, weight, "
	...
	List<ProductDTO> getAll(@Param("start") int start, @Param("end") int end, @Param("where") String where,
	...
	int getTotal(@Param("where") String where, @Param("ctno") String ctno, @Param("ctno1") String ctno1,
			@Param("ctno2") String ctno2);
	...
	@Update({
			"update product set brand = #{dto.brand}, name = #{dto.name}, subject = #{dto.subject}, sel_unit = #{dto.sel_unit}, weight = #{dto.weight}, "
	...
	@Select({
			"select pno, price, discount, earn, ctno, ctno1, ctno2, quantity, name, brand, subject, sel_unit, weight, pack_type, info, "
					...
					+ "(select p.*, f.name jname, f.loc jloc from product p left join files f on REGEXP_SUBSTR(p.fno, '[^,]+', 1, 1) = f.fno "
					+ "${where} order by p.writedate desc) t) tt where seq >= #{start}) where rownum <= #{end}" })
	List<ProductFilesDTO> getEventFile(@Param("start") int start, @Param("end") int end, @Param("where") String where,
			@Param("brand") String brand, @Param("name") String name);

	@Select("select count(*) from product ${where}")
	int getEventTotal(@Param("where") String where, @Param("brand") String brand, @Param("name") String name);
