...
public class OrderController {
	...
	public String order(Model model, HttpServletRequest request, PrintWriter out)
	{	
		String id = "henry";
		...
		for (int i = 0;i<basketArr.size();i++) 
		{
			...
			if (basketArr.get(i).getPono() == 0 ) 
			{
				...
				if(basketArr.get(i).getDiscount() > 0) 
				{
					...
					pdiscount += (basketArr.get(i).getPprice()*((basketArr.get(i).getDiscount()/100)))*basketArr.get(i).getQuantity();
					ppriceWithDiscount += ((basketArr.get(i).getPprice()-(basketArr.get(i).getPprice()*((basketArr.get(i).getDiscount()/100)))))*basketArr.get(i).getQuantity();
				}
				//할인이 없으면
				else 
				{
			...
			else if (basketArr.get(i).getPono() > 0)
			{
				...
				if(basketArr.get(i).getDiscount() > 0) 
				{
					...
					odiscount += (basketArr.get(i).getOprice()*((basketArr.get(i).getDiscount()/100)))*basketArr.get(i).getQuantity();
					opriceWithDiscount += ((basketArr.get(i).getOprice()-(basketArr.get(i).getOprice()*((basketArr.get(i).getDiscount()/100)))))*basketArr.get(i).getQuantity();
				}
				else 
				{
...
//		
		...
		for(int a = 0;a<allOkCoupon.size();a++) 
		{
			...
			if(allOkCoupon.get(a).getDis_type().equals("d")) 
			{
				...
				else if(totalPrice  > totalPriceWithDiscount) 
				{
					...
					allOkCouponDiscountPrice = (int)Math.round(totalPriceWithDiscount*(allOkCoupon.get(a).getDiscount()/100));
				}
			}
			//만약에 할인의 type이 p일 경우
		...
		for(int e = 0;e<productOkCoupon.size();e++) 
		{
			...
			if(productOkCoupon.get(e).getDis_type().equals("d")) 
			{
				...
				pnoAppDiscountPrice = (int)Math.round(pnoAppTotalPriceArr.get(e)*(productOkCoupon.get(e).getDiscount()/100));
			}
	...
	public String addrUpdate(Model model) 
	{
		//기본 배송지 가져오기
		String mno = "21";
		DeliveryDTO ddto = deliveryMapper.getDefault(mno);
	...
	public String addrUpdateok(Model model, PrintWriter out,HttpServletRequest request) 
	{
		...
		deliveryMapper.updateAddr(receive_name,phone,pickUpType,pickUpTypeContent,pickUpTypeDetail,deliveryMsg,mno);
	...
	public String orderOk(PrintWriter out, Model model, HttpServletRequest request) 
	{
		...
		//결제 테이블에 내용 삽입
		paymentMapper.insertPayment(mno,ono,type,payment,creditUsed,productTotalPrice,deliveryPrice,productDisPrice,couponDisPrice,creditEarned,cardName,cardMonth);
