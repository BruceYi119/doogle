...
//		model.addAttribute("list", gradeMapper.getNext(id));
	...
	public ModelAndView event_product(ModelAndView mv, HttpServletRequest request, EventDTO edto) {
		...
		paging.setPaging(page, productMapper
				.getEventTotal("where brand like '%'||#{brand}||'%' and name like '%'||#{name}||'%'", brand, name),
				"/shop/event/product?eno=" + edto.getEno() + "&brand=" + brand + "&name=" + name);
		System.out.println(productMapper
				.getEventTotal("where brand like '%'||#{brand}||'%' and name like '%'||#{name}||'%'", brand, name));
		mv.addObject("plist", productMapper.getEventFile(paging.getStartRow(), paging.getViewCnt(),
				"where p.brand like '%'||#{brand}||'%' and p.name like '%'||#{name}||'%'", brand, name));
