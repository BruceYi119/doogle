...
//	자주하는 질문
	...
	public String questionList(Model model, HttpServletRequest request, QuestionDTO dto) {
		...
		if (dto.getCtno() == null) {
			...
			paging.setPaging(page, questionMapper.getTotal("where ctno =#{ctno}", dto.getCtno()),
					"/shop/questionList?ctno=" + dto.getCtno());
			model.addAttribute("list", questionMapper.getAll(paging.getStartRow(), paging.getViewCnt(),
					"where ctno = #{ctno}", dto.getCtno()));
		}
